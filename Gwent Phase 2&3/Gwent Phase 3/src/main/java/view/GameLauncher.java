...
public class GameLauncher extends Application {
    ...
    public boolean enemyIsDone = false;
    private Deck deck;
    private  Stage stage;
...
//            System.out.println("has passed: " +hasPassed);
    ...
    private void createGlobalChat() {
        globalChat = new Pane();
        chatsVBox = new VBox();
        chatsVBox.setLayoutY(40);
        chatsVBox.setLayoutX(10);
        chatsVBox.setMinHeight(360);
        chatsVBox.setMaxHeight(360);
        chatsVBox.setMaxWidth(280);
        chatsVBox.setMinWidth(280);
        globalChat.setOnScroll(scrollEvent -> {
            double constant = scrollEvent.getDeltaY() < 0 ? 1 : -1;
            double y = chatsVBox.getLayoutY() + 25 * constant;
            if (y <= 40 && y >= -250)
                chatsVBox.setLayoutY(y);
            handleScroll();
        });
        chatField = new TextField();
        chatField.setPrefWidth(260);
        chatField.setPrefHeight(25);
        chatField.setLayoutY(410);
        chatField.setLayoutX(20);
        Button sendText = new Button("send");
        sendText.setLayoutX(240);
        sendText.setLayoutY(410);
        Rectangle close = new Rectangle(10,10,20,20);
        close.setFill(new ImagePattern(new Image(MessagePane.class.getResource("/Images/icons/anim_bond.png").toExternalForm())));
        ...
        globalChat.setBorder(Border.stroke(Color.BLACK));
        globalChat.setMaxWidth(300);
        globalChat.setMinWidth(300);
        globalChat.setMinHeight(450);
        globalChat.setMaxHeight(450);
        globalChat.setLayoutY(200);
        globalChat.setLayoutX(1250);
        Image image = new Image(Game.class.getResource("/Images/mainmenubackground1.jpg").toExternalForm(), WIDTH ,HEIGHT, false, false);
        BackgroundImage backgroundImage = new BackgroundImage(image,
                BackgroundRepeat.NO_REPEAT,
                BackgroundRepeat.NO_REPEAT,
                BackgroundPosition.DEFAULT,
                BackgroundSize.DEFAULT);
        globalChat.setBackground(new Background(backgroundImage));
        globalChat.setVisible(false);
    }

    private void handleScroll() {
        for (Node node : chatsVBox.getChildren())
            node.setVisible(false);
        int upperBound = chatsVBox.getChildren().size() <= 8 ? chatsVBox.getChildren().size() :
                chatsVBox.getChildren().size() -8 - (int)chatsVBox.getLayoutY()/40;
        for (int i = 1 - (int)chatsVBox.getLayoutY() / 40 ; i < upperBound ; i ++) {
            chatsVBox.getChildren().get(i).setVisible(true);
        }
    }

    public void addMessageToGlobalChat(String text, String repliedText, String sender, String  repliedGuy){
        MessagePane message = new MessagePane(text, repliedText, sender, repliedGuy);
        message.reply.setOnMouseClicked(event -> {
            addMessageToGlobalChat(chatField.getText(), message.text, User.getLoggedInUser().getUsername(), message.sender);
    ...
    private void sendReaction(int i) {
        ...
        fadeTransition.play();
        client.sendMessage("reaction:"+User.getLoggedInUser().currentOponentName+":"+i+".reaction");
    }
